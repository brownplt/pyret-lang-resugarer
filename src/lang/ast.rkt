...
(define-struct srcloc (source line column position span tags) #:transparent)
(define (empty-srcloc phantom-filename)
  (srcloc phantom-filename #f #f #f #f (list)))
...
(define (reify-srcloc s)
  ...
  (match s
    [(srcloc src line col pos span _)
...
(define (s-stmt? x)
  (let [[stmt-types (list
     s-let? s-fun? s-data? s-when?
     s-var? s-assign? s-graph? s-check?)]]
