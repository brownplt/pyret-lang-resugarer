...
(require
  ...
  "indentation.rkt"
  "compile.rkt"
  (prefix-in resugarer: "compile-resugared.rkt")
  "load.rkt"
  "pretty.rkt"
  "runtime.rkt")
...
(define (stx->racket
  ...
  (define resugar-mode (current-resugar-mode))
  (define desugar
    (cond
      [resugar-mode (lambda (e) (resugarer:desugar resugarer-dir "Prog" e))]
      [check (lambda (e) (desugar-pyret (desugar-check e)))]
      [else desugar-pyret]))
  (define compile
    (if resugar-mode
        (if toplevel resugarer:compile-pyret resugarer:compile-expr)
        (if toplevel compile-pyret compile-expr)))
